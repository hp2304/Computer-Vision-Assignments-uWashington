...
void gradient_matrix(matrix m, ACTIVATION a, matrix d)
{
    ...
    for(i = 0; i < m.rows; ++i){
        for(j = 0; j < m.cols; ++j){
            double x = m.data[i][j], del = d.data[i][j];
            // TODO: multiply the correct element of d by the gradient
            if(a == LOGISTIC){
                d.data[i][j] = del*x*(1-x);
            } else if (a == RELU){
                d.data[i][j] = x>0?del:0;
            } else if (a == LRELU){
                d.data[i][j] = x>0?del:(0.01*del);
            } else if (a == SOFTMAX){
                d.data[i][j] = del; //*x*(1-x);
            }
        }
...
void update_layer(layer *l, double rate, double momentum, double decay)
{
    ...
    // save it to l->v
    matrix del_w_t = axpy_matrix(momentum, l->v, axpy_matrix(-decay, l->w, l->dw));
    free_matrix(l->v);
    l->v = del_w_t;

    // Update l->w
    matrix w_t1 = axpy_matrix(rate, del_w_t, l->w);
    free_matrix(l->w);
    l->w = w_t1;
